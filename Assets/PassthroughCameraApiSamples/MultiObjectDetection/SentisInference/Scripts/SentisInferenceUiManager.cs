...
namespace PassthroughCameraSamples.MultiObjectDetection
{
    ...
    public class SentisInferenceUiManager : MonoBehaviour
    {
        ...
        // Cache de reflection para evitar coste por frame
        private MethodInfo _vp2Pose;
        private MethodInfo _vp3Pose;
        private MethodInfo _vp2;
        private MethodInfo _vp3;

        private void Awake()
        {
            m_detectionBoxPrefab.gameObject.SetActive(false);
            CacheViewportPointToRayMethods();
        }

        private void CacheViewportPointToRayMethods()
        {
            if (m_cameraAccess == null) return;

            Type t = m_cameraAccess.GetType();
            var flags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;

            // Buscamos overloads posibles:
            // ViewportPointToRay(Vector2, Pose)
            _vp2Pose = t.GetMethod("ViewportPointToRay", flags, null, new[] { typeof(Vector2), typeof(Pose) }, null);
            // ViewportPointToRay(Vector3, Pose)
            _vp3Pose = t.GetMethod("ViewportPointToRay", flags, null, new[] { typeof(Vector3), typeof(Pose) }, null);

            // ViewportPointToRay(Vector2)
            _vp2 = t.GetMethod("ViewportPointToRay", flags, null, new[] { typeof(Vector2) }, null);
            // ViewportPointToRay(Vector3)
            _vp3 = t.GetMethod("ViewportPointToRay", flags, null, new[] { typeof(Vector3) }, null);
        }

        private Ray ViewportPointToRayCompat(Vector2 viewport01, Pose cameraPose)
        {
            if (m_cameraAccess == null)
            {
                // Fallback (no debería pasar)
                return new Ray(cameraPose.position, cameraPose.forward);
            }

            // Prioridad: overloads con pose (más correctos)
            if (_vp2Pose != null)
            {
                object r = _vp2Pose.Invoke(m_cameraAccess, new object[] { viewport01, cameraPose });
                return (Ray)r;
            }

            if (_vp3Pose != null)
            {
                object r = _vp3Pose.Invoke(m_cameraAccess, new object[] { new Vector3(viewport01.x, viewport01.y, 0f), cameraPose });
                return (Ray)r;
            }

            // Si tu SDK nuevo ya no necesita Pose, usa el overload de 1 parámetro
            if (_vp2 != null)
            {
                object r = _vp2.Invoke(m_cameraAccess, new object[] { viewport01 });
                return (Ray)r;
            }

            if (_vp3 != null)
            {
                object r = _vp3.Invoke(m_cameraAccess, new object[] { new Vector3(viewport01.x, viewport01.y, 0f) });
                return (Ray)r;
            }

            // Último fallback: ray desde pose
            Debug.LogWarning("No se encontró ningún overload de ViewportPointToRay en PassthroughCameraAccess. Usando fallback.");
            return new Ray(cameraPose.position, cameraPose.forward);
        }
        ...
        public void DrawUIBoxes(List<(int classId, Vector4 boundingBox)> detections, Vector2 inputSize, Pose cameraPose)
        {
            ...
            for (var i = 0; i < detections.Count; i++)
            {
                ...
                Vector2 normalizedCenter = rect.center / inputSize; // 0..1 en espacio de input
                ...
                // Convertimos a viewport 0..1 y corregimos Y (dependiendo de cómo venga el sample)
                Vector2 viewportCenter = new Vector2(normalizedCenter.x, 1.0f - normalizedCenter.y);

                // Get the 3D marker world position using Depth Raycast
                var ray = ViewportPointToRayCompat(viewportCenter, cameraPose);
                ...
                Vector2 vpCenter = normRect.center;
                Vector2 vpMin = normRect.min;
                Vector2 vpMax = normRect.max;

                var worldSpaceCenter = ViewportPointToRayCompat(vpCenter, cameraPose).GetPoint(distance);
                ...
                var minRay = ViewportPointToRayCompat(vpMin, cameraPose);
                var maxRay = ViewportPointToRayCompat(vpMax, cameraPose);
